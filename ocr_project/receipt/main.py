...
def process_receipt_image(image_path):
    ...
    # Load the image, resize it, and compute the ratio
    ...
    for c in cnts:
        ...
        peri = cv2.arcLength(c, True)
        approx = cv2.approxPolyDP(c, 0.02 * peri, True)
        # If our approximated contour has four points, assume we found the receipt outline
        if len(approx) == 4:
            receiptCnt = approx
            break

    if receiptCnt is None:
        raise Exception("Could not find receipt outline. Check edge detection and contours.")

    # Apply a four-point perspective transform to obtain a top-down view of the receipt
    receipt = four_point_transform(img_orig, receiptCnt.reshape(4, 2) * ratio)

    # Apply OCR to the transformed receipt image
    options = "--psm 6"
    text = pytesseract.image_to_string(cv2.cvtColor(receipt, cv2.COLOR_BGR2RGB), config=options)

    # Format the OCR text as JSON
    result = json.dumps({"text": text}, indent=4)

    # Output the JSON-formatted OCR text
    # print(result)
    return result